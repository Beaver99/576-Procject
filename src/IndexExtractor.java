...
// T1 += getThreshold(firstFrame, 34);
// T2 += getThreshold(firstFrame, 33);
// T3 += getThreshold(firstFrame, 33);
//
// }
//
// T1 /= sampleCount;
...
// float t1 = hod(prev_Scene, currFrame);
// float t2 = hod(prev_Shot, currFrame);
                    float t3 = hod(prev_Subshot, currFrame);
                    if (t3 >= T1) {
    ...
    public static float test() {
        ...
        Arrays.fill(currFrame, 0);
        Arrays.fill(prevFrame, 5);
        return histogramDifference(currFrame, prevFrame);
    }

    public static float hod(int[] prev, int[] curr) {
        ...
        for (int i = 0; i < numBlocks; i++) {
            ...
            for (int ii = 0; ii < N; ii++) {
                ...
                for (int jj = 0; jj < N; jj++) {
                    ...
                    blockDiff[ii * N + jj] = curr[idx] - prev[idx];
                }
            }
            diffHistogram[i] = hodCalculateHistogram(blockDiff, numBins);
        }
...
// step2: calculate the difference using the Block histogram difference (BH)
        ...
        for (int i = 0; i < numBlocks; i++) {
            float blockDifference = hodCalculateBlockDifference(diffHistogram[i], N);
...
//        System.out.println(averageDifference);
    ...
    private static int[] hodCalculateHistogram(int[] block, int numBins) {
        int[] histogram = new int[numBins];
        for (int i = 0; i < block.length; i++) {
            histogram[(block[i] + 255) * (numBins - 1) / (2 * 256 - 1)]++;
        }
        return histogram;
    }

    private static float hodCalculateBlockDifference(int[] diffHistogram, int N) {
        int similarCount = 0;
        for (int i = 255 - 31; i < 255 + 32; i++) {
            similarCount += diffHistogram[i];
        }
        return 1.0f - ((float) similarCount) / (float) (N * N);
    }
